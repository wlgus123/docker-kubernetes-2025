Dockerfile은 **Docker 이미지**를 생성하기 위한 **명령어와 설정 파일**입니다. 각 명령어는 이미지의 **계층(layer)**을 형성하며, Docker는 이를 조합해 최종 이미지를 만듭니다.
...
## ⚡ **Dockerfile 작성 시 Best Practices**  

| 🎯 팁                        | 💡 설명                                       |
|--------------------------|-------------------------------------------|
| ✅ **작은 베이스 이미지 사용**   | `node:alpine`, `python:slim`으로 용량 최소화            |
| ✅ **멀티 스테이지 빌드**       | 빌드 환경과 런타임 환경을 분리해 불필요한 파일 제거        |
| ✅ **캐싱 활용**               | 자주 변경되는 파일은 `COPY` 명령을 아래에 배치하여 빌드 속도 향상 |
| ✅ **보안 고려**               | `USER` 명령으로 루트 계정 대신 일반 사용자 실행             |
| ✅ **불필요한 파일 삭제**       | `RUN apt-get clean && rm -rf /var/lib/apt/lists/*` 사용   |
